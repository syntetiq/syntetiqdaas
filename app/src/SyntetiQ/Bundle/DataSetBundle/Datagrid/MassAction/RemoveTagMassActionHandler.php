...
class RemoveTagMassActionHandler implements MassActionHandlerInterface
{
    ...
    public function handle(MassActionHandlerArgs $args): MassActionResponseInterface
    {
        ...
        return new MassActionResponse(
            ...
            $this->translator->trans(
                $updatedCount > 0
                    ? 'Tag "%tag%" removed from %count% item(s).'
                    : 'No selected items contain tag "%tag%".',
                [
                    '%tag%' => $tagValue,
                    '%count%' => $updatedCount,
                ]

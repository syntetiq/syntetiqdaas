 ...
 * create select field
 */
class WidgetDataSetSelectType extends WidgetEntityJquerySelect2HiddenType
{
    const NAME = 'oro_type_widget_data_set_select';
    ...
    protected function getEntitiesByIdentifiers(string $entityClass, array $ids): array
    {
        ...
        $key = array_search(OwnerHelper::CURRENT_USER, $ids, true);
        if ($key !== false) {
            unset($ids[$key]);
        }
